...
export async function createUser(data: DbUser) {
    await sql`INSERT INTO users (displayname, description, email, profileimage) VALUES ('${data.displayName}', '${data.description}', '${data.email}', '${data.profileImage}');`;
}

export async function ownsOrg(userId: string, orgId: string) {
    const rows = await sql`
SELECT COUNT (*) FROM oragnisationmanagers as om 
